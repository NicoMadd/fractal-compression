...
- **Experiment image set:** [`data/experiments/`](data/experiments/) — frozen inputs for the paper's evaluation, built by [`generate_experiment_assets.py`](data/experiments/generate_experiment_assets.py) (Pillow). Four **512×512** images: **`apollonian_gasket_512`** (center crop of the original **600×600** `apollonian_gasket.ascii.pgm`), **`lena`**, **`baboon`**, and **`sun_tzu_512`** (center crop of **593×1024** `sun_tzu.ascii.pgm`). Per image the script emits, under **`images/`**, **`pgma/<stem>.ascii.pgm`** (P2, levels copied verbatim so `maxval` never rescales samples), then **`png/<stem>.png`** and **`jpeg/<stem>_q{25,50,75,90}.jpeg`** re-read from that PGMA (no chroma subsampling), plus **`assets_manifest.json`** with dimensions, byte sizes and **bits per pixel** for every artifact.
...
- **Experiment run harness:** [`data/experiments/run_fractal_experiments.sh`](data/experiments/run_fractal_experiments.sh) runs the paper's matrix via [`run.sh`](implementations/java/compression/run.sh) (`IMAGES_ROOT=data/experiments/images`): four PGMs × **r4/d8**, **r8/d16**, **r16/d32**, **`-i 20`**, **12** threads, fresh codebook (**`-c`**) per run; archives **`benchmark.csv`** and **`run_manifest.json`**.
- **Reference measurements:** [`measure_reference_metrics.py`](data/experiments/measure_reference_metrics.py) computes MSE/MAE/PSNR, bpp, ratio and median encode/decode times for the PNG and JPEG references against the PGM original → **`reference_metrics.json`**; [`summarize_results.py`](data/experiments/summarize_results.py) joins fractal and reference runs into **`results_summary.json`** and emits the LaTeX rows.
- **Paper section:** [`docs/paper/resultados.tex`](docs/paper/resultados.tex) — "Resultados" with measured data only (no interpretation): one table with ratio, bpp, PSNR at iterations **5**, **10** and **20**, and encode/decode times for FBC **r4/d8**, **r8/d16** and **r16/d32**, PNG and JPEG 25–90, across Apollonian, Lena, Baboon and Sun Tzu.
- **Rate–distortion figure:** [`plot_results.py`](data/experiments/plot_results.py) renders **`docs/paper/images/psnr_vs_bpp.png`** — PSNR vs bits per pixel per image, fractal and JPEG points labelled, PNG drawn as a vertical rate marker.
- **Rate accounting for the paper:** a "Medición del tamaño comprimido" subsection documents the textual **`.fc`** serialization (seven space-separated fields per assignment, ~29 bytes each, with redundant range coordinates and unquantized coefficients) and reports its on-disk size as the fractal rate. Compression ratio is now defined against the uncompressed **8 bpp** image rather than the **PGM ASCII** input, which stores pixels as text and inflates every ratio by roughly **4×**. Under that reference **r4/d8** does not compress at all (**0,54–0,62:1**, i.e. the codebook is larger than the raw image) while **r8/d16** reaches **~2,2:1**.
- **Results section:** [`docs/paper/resultados.tex`](docs/paper/resultados.tex) — comparison table across the four images, three fractal geometries and the PNG/JPEG references, introduced by a two-sentence lead.
- **Discussion section:** [`docs/paper/discusion.tex`](docs/paper/discusion.tex) — rate, quality, timings, comparison against the references and limitations, including the non-monotonic PSNR of Apollonian at **r8/d16** (convergence is not guaranteed because the contrast coefficient is unbounded) and the MAE/MSE divergence caused by its thin edges on a uniform background.
...
- **C++ `run.sh`:** incremental compile — skip **`g++`** when **`main.exe`** is newer than all **`*.cpp`** sources; **`--fc`** or **`FORCE_COMPILE=1`** forces a rebuild; **`--fc`** is not passed through to **`main.exe`**; links **miniz** **`.c`** object files, **`-Wno-deprecated-declarations`** for **stb**; **`set -e`** without **`set -u`** so a no-arg **`./run.sh`** does not treat an empty **`pass_args`** as unbound.
- **`--debug`:** verbose stdout (phase/progress logs). Default is **summary-only** (`--- Summary ---` with timings, ratio, metrics, output paths).
- **C++ decode metrics:** **`image/error_metrics.hpp`** / **`error_metrics.cpp`** — **`computeErrorMetricsFromPgmPaths`** re-reads the input PGM and each written **`iter_*.pgm`** (like Java **`PGMAUtils.calculateErrorMetrics`**); in-memory overlap uses **`min(rows)×min(cols)`** when reference and reconstruction differ; per-iteration **stdout** + **`processes/cpp/<stem>/iterations/benchmark.csv`** (Java **`Iteration`** columns); **`DecompressResult`** from **`decompress`**; pipeline takes **`originalImagePath`**; **`file_paths::copyOriginalToRunDir`** writes **`runDir/original.pgm`**. **`pgma::read_matrix`** in **`image/metadata.*`** for loading **P2** to **`Matrix<GrayPixel>`** without a leaking **`PGMAImageMetadata`**.
- **C++ benchmark baselines (Java `CompressionBaselines` style):** **`utils/compression_baselines.hpp` / `.cpp`** — **`baselines/`** lossless **PNG** of **`original.pgm`**, reconstruction from **`iter_<n>.pgm`** when present, and **DEFLATE** **`.zip`** archives of **`original.pgm`** and the on-disk **`.fc`** (miniz, level 9); **stdout** prints compression ratios (PGM vs raw codebook, vs deflated codebook) plus lines vs **lossless PNG**, **DEFLATE of PGM**, deflated codebook, and **(raw `.fc` + deflated-zip) total** bytes, each with **Δ** vs the raw-codebook ratio, plus final **MSE**/**MAE**/**PSNR** and the baselines directory path. Third-party: **`implementations/cpp/third_party/`** (**miniz** 3.0.2, **stb_image_write**, **`miniz_export.h`** for static build).
- **C++ `SequenceReader::readNextFloat`** and **`fractal/mapping/codebook`**: read **`.fc`** mapping **`s` / `o`** as **IEEE-754** tokens (aligns on-disk codebook with Java `SequenceReader`).
- **Registry:** more **Java** **`lena.ascii`** append-only lines in [**`benchmarks/registry.jsonl`**](benchmarks/registry.jsonl).
...
- **C++ CLI / pipeline:** **`-c`** forces codebook rebuild (same idea as Java **`cleanCodebook`**). Otherwise, if **`processes/cpp/<stem>/codebooks/codebook_r{r}_d{d}.fc`** exists and header **`r`/`d`** match the run, mappings are **loaded** and compression is skipped; new codebooks use the same **`codebooks/`** path as Java’s **`Codebook.pathForGeometry`**.
- **C++ `DomainFinder`:** **`findBest`** returns **`FractalMapping`**; one **`Matrix<GrayPixel>`** scratch per search; **`calculate_s`** takes **`Block*`** range plus transformed reduced-domain pixels so LS and MSE use the same range window; winner stored as domain grid indices with **`(s, o, transformation_type)`**.
- **C++ `Decompressor::apply`:** applies the mapping’s **`TransformationType`** to the reduced domain before affine **`s`** / **`o`** (decode matches encode for non-identity transforms).
- **C++ `GrayBlockCompression` / `PGMAPipeline`:** refactored mapping discovery, **`FractalMapping`** / **`Codebook`** wiring, and parallel **`Executor`** submission for range tasks.
- **C++ `run.sh`:** prints **`Running ./main.exe…`**, or **`(verbose: --debug)`** when **`--debug`** is passed (replaces misleading “debug mode” wording).
- **Java fractal transforms (documentation):** expanded Javadoc on **`Transformation`**, **`TransformationType`**, and each concrete transform (row/column indexing, **`HZ`** / **`VC`** / diagonals); added **`package-info.java`** under **`utils/fractal/transformation`**.
...
- **C++ pipeline:** writes per-geometry codebook under **`codebooks/`**, iteration frames as **`iterations/iter_<n>.pgm`**, **`iterations/benchmark.csv`**, and **`final.pgm`** under **`processes/cpp/<image-stem>/`** (cwd **`implementations/cpp`**); **`iterations/`** is removed and recreated each run before decode; P2 save uses **width = columns, height = rows** to match Java; **`decompress`** returns **`DecompressResult`**; final **MSE**/**MAE**/**PSNR** and compression-ratio block run after decode via **`compression_baselines::printRunBaselinesAndCompressionRatios`** (not only **`run_logging::debug`**).
- **C++ `PipelineParams` / `printUsage`:** documents **`-c`** / **`cleanCodebook`** (force codebook rebuild).
...
- **C++ `DomainFinder` least-squares:** range side uses **`Block::get(i, j)`** (block-relative indices into the full image window), not **`range->pixels->get(i, j)`** on the top-left of the frame, so **`s`** / **`o`** match the MSE loop and Java’s **`DomainFinder`** behavior.
